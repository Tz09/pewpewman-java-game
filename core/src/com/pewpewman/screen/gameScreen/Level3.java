...
public class Level3 extends AbstractScreen {
    ...
    private final TextureAtlas.AtlasRegion bossBullet;
    ...
    public Level3(Pewpewman game) {
        ...
        TextureAtlas bossAtlas   = game.getAssetManager().get("packed/boss/textures.atlas", TextureAtlas.class);

        floorTile1   = elementAtlas.findRegion("level3_tile1");
        floorTile2   = elementAtlas.findRegion("level3_tile2");
        wall         = elementAtlas.findRegion("level3_wall");
        obstacle     = elementAtlas.findRegion("level3_obstacle");
        gate         = elementAtlas.findRegion("gate");
        enemyBullet  = elementAtlas.findRegion("enemy_bullet");
        bossBullet  = elementAtlas.findRegion("boss_bullet");
        ...
        AnimationSet bossAnimations = new AnimationSet(
                new Animation(0.1f, bossAtlas.findRegions("boss_walk_north"), Animation.PlayMode.LOOP_PINGPONG),
                new Animation(0.1f, bossAtlas.findRegions("boss_walk_south"), Animation.PlayMode.LOOP_PINGPONG),
                new Animation(0.1f, bossAtlas.findRegions("boss_walk_east"), Animation.PlayMode.LOOP_PINGPONG),
                new Animation(0.1f, bossAtlas.findRegions("boss_walk_west"), Animation.PlayMode.LOOP_PINGPONG),
                bossAtlas.findRegion("boss_stand_north"),
                bossAtlas.findRegion("boss_stand_south"),
                bossAtlas.findRegion("boss_stand_east"),
                bossAtlas.findRegion("boss_stand_west")
        );
        ...
        int enemyStartX;
        int enemyStartY;
        for (int i = 0; i < Settings.ENEMY_LEVEL3; i++) {
            if (i == 0) {
                enemyStartX = Settings.MAP_CENTRE_X - 4;
                enemyStartY = Settings.MAP_CENTRE_Y;
            } else if (i == 1) {
                enemyStartX = Settings.MAP_CENTRE_X + 4;
                enemyStartY = Settings.MAP_CENTRE_Y;
            } else if (i == 2) {
                enemyStartX = Settings.MAP_CENTRE_X;
            ...
            if (i == Settings.ENEMY_LEVEL3-1) {
                enemy.add(new Enemy(map, enemyStartX, enemyStartY, bossAnimations, ENEMY_TYPE.BOSS));
            } else {
        ...
        // Enter text here
        openingBox.createStoryBoxUI("Be prepared Pewpewman,\n\n\n" +
                "It is impossible for you to beat this stage,\n\n\n" +
                "But if you're ready to accept the truth...\n\n\n" +
                "press X and DIEEEE!!!!!");
        endingBox.createStoryBoxUI("Congratulations Pewpewman,\n\n\n" +
                "You have save the world...\n\n\n" +
                "and I hope that our future path will crossed again!\n\n\n" +
                "Enter the gate and leave this world, Pewpewman!");
    }
...
//        }
        ...
        if(storyEnd && enemyShootTimer >= Settings.ENEMY_SHOOT_WAIT_TIME)
        {
            ...
            {
                if (enemies.getEnemyType() == ENEMY_TYPE.NORMAL) {
                    enemyBullets.add(new Bullet(enemies.getX(), enemies.getY(), Settings.ENEMY_BULLET_SPEED,enemies.getFacing(), enemyBullet));
                } else {
                    enemyBullets.add(new Bullet(enemies.getX(), enemies.getY(), Settings.ENEMY_BULLET_SPEED, DIRECTION.NORTH, bossBullet));
                    enemyBullets.add(new Bullet(enemies.getX(), enemies.getY(), Settings.ENEMY_BULLET_SPEED, DIRECTION.SOUTH, bossBullet));
                    enemyBullets.add(new Bullet(enemies.getX(), enemies.getY(), Settings.ENEMY_BULLET_SPEED, DIRECTION.EAST, bossBullet));
                    enemyBullets.add(new Bullet(enemies.getX(), enemies.getY(), Settings.ENEMY_BULLET_SPEED, DIRECTION.WEST, bossBullet));
                }
            }
        }
        ...
        if (storyEnd) {
            ...
            if (enemy.size() == 0) {
                ...
                if (playVictoryMusic == 0) {
                    levelEnd = true;
                    musicManager.playMusic(TRACK.FINISHGAME);
                    playVictoryMusic++;
